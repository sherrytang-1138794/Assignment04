...
def test_database_manager_delete_bookmark(database_manager):
    ...
    # act
    database_manager.add("bookmarks", dataOne)
    database_manager.add("bookmarks", dataTwo)

    # assert
    conn = database_manager.connection
    cursor = conn.cursor()
    cursor.execute(''' SELECT * FROM bookmarks WHERE title='test_title_one' ''')    
    assert cursor.fetchone()[0] == 1   

    database_manager.delete("bookmarks", dataOne) 

    cursor.execute(''' SELECT * FROM bookmarks WHERE title='test_title_two' ''')    
...
def test_database_manager_select_bookmark(database_manager):
    ...
    database_manager.add("bookmarks", dataOne)
    database_manager.add("bookmarks", dataTwo)
    selection_with_criteria = database_manager.select("bookmarks", criteria={"id":1}, order_by=None)
    selection_with_order = database_manager.select("bookmarks", order_by="title")
    # assert
    conn = database_manager.connection
    cursor = conn.cursor()
    cursor.execute(''' SELECT * FROM bookmarks WHERE ID=1 ''')    
    assert cursor.fetchone()[0] == list(selection_with_criteria)[0][0] == 1

    cursor.execute(''' SELECT * FROM bookmarks ORDER BY title ''')    
    assert cursor.fetchall() == list(selection_with_order)
